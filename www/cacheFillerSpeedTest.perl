...
{	
	...
	while( $_ = <UNKNOWN> )
	{
		...
		if( $linkDepth < $commandline{'maxdepth'} )
		{
		...
		if( $commandline{'htmlpages'} eq 'save' )
		{
			...
			`mv html/$site/$path html/$site/$path.wrong; mkdir html/$site/$path;mv html/$site/$path.wrong html/$site/$path/index.html` if -f  "html/$site/$path";
		...
		if( $linkDepth <= $commandline{'maxdepth'} )
		{
			...
			{
				$_ = $newlink;
				if( /^http:\/\// && /http:\/\/[a-z\.]*[:\/]/ && (!defined($visited{$newlink})) )
				{
