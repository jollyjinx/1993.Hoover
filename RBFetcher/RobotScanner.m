...
+ (void)initialize;
{
    ...
    if( ! ( userAgentName = [[hooverConfigurationDictionary objectForKey:@"useragentname"] retain] ) )
    {
...
{
    [super init];
    includedPathArray = incArray;
    excludedPathArray = excArray;
    if( includedPathArray ) [includedPathArray retain];
    if( excludedPathArray ) [excludedPathArray retain];
