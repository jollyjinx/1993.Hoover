...
int main (int argc, const char *argv[])
{
    ...
    if( connectionObject = [NSConnection connectionWithRegisteredName:CONNECTIONNAME host:hostName])
    {
        ...
        [connectionObject setIndependentConversationQueueing:YES];
        [connectionObject enableMultipleThreads];
        ...
        localFetcher = [[Fetcher alloc] initWithHooverObject:hooverObject availableConnections:maximumthreads];
        [hooverObject addFetcher:localFetcher];
        [(NSDistantObject *)hooverObject setProtocolForProxy:@protocol(FetcherControllerProtocol)];

        maximumthreads--;
        while(maximumthreads--)
        {
            [connectionObject runInNewThread];
        }
