               ...
               lock:(NSLock *)crashMachineLock;
{
...

    {
        ...
        void	*datapointer;
        int	datalength;
        momData = [requestString dataUsingEncoding:NSISOLatin1StringEncoding];
        datapointer = [momData bytes];
        datalength = [momData length];
        if( -1 == write(httpsocket,datapointer,datalength) )
            [NSException raise:@"FetcherSocket" format:@"Did not write any data to the server."];
    }

    httpData = [NSMutableData dataWithCapacity:10000];
    beginDate = [NSDate date];
    {
        int sizeread;
        char buffer[1000];
        
        while( (sizeread = read(httpsocket,buffer,1000)) >0 )
        {
            [httpData appendBytes:buffer length:sizeread];
        }
        shutdown(httpsocket,2);
        close(httpsocket);        
    }
    ...
    [url setObject:[NSNumber numberWithFloat:(float)([httpData length])/(float)((float)[endDate timeIntervalSinceDate:beginDate]+0.1)]
            forKey:@"transferrate"];
    [url setObject:[NSNumber numberWithFloat:(float)[endDate timeIntervalSinceDate:beginDate]]
            forKey:@"transfertime"];
...
#endif
    [crashMachineLock lock];
    [hooverObject retrievedUrl:url];
    [crashMachineLock unlock];
}
