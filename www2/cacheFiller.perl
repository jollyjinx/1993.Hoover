...
while($_=pop(@ARGV))
{
	...
	$commandline{'storagenumber'}=$1 	if /^storagenumber=(.*)/;
...
if($commandline{'languages'} eq 'remove')
{
	...
	`rm -f language.url.gdbm`;
	`rm -f language.word.gdbm`;
	`rm -f language.bigram.gdbm`;
	`rm -f language.trigram.gdbm`;
	print "done.\n";
}

if( $commandline{'languages'} ne 'none')
{
	dbmopen(%physicalLanguageUrlDatabase,'language.url.gdbm',0666) || die "Can't open languageUrlDatabase\n";
	dbmopen(%physicalLanguageWordDatabase,'language.word.gdbm',0666)|| die "Can't open languageWordDatabase\n";
	dbmopen(%physicalLanguageBigramDatabase,'language.bigram.gdbm',0666)|| die "Can't open languageBigramDatabase\n";
	dbmopen(%physicalLanguageTrigramDatabase,'language.trigram.gdbm',0666)|| die "Can't open languageTrigramDatabase\n";

	%languageUrlDatabase		= %physicalLanguageUrlDatabase;
	%languageWordDatabase		= %physicalLanguageWordDatabase;
	%languageBigramDatabase		= %physicalLanguageBigramDatabase;
	%languageTrigramDatabase	= %physicalLanguageTrigramDatabase;
	
	dbmclose(%physicalLanguageUrlDatabase);
	dbmclose(%physicalLanguageWordDatabase);
	dbmclose(%physicalLanguageBigramDatabase);
	dbmclose(%physicalLanguageTrigramDatabase);

	undef %physicalLanguageUrlDatabase;
	undef %physicalLanguageWordDatabase;
	undef %physicalLanguageBigramDatabase;
	undef %physicalLanguageTrigramDatabase;
...
{	
	...
	while( ($url,$value) = each(%unknownLink) )
	{
			...
			&& (@sentences = &htmlToSentences($contents)) )	
		{
			...
			if( 0 == ($languageUrlCount % $commandline{'storagenumber'}) )
			{
				&savelinkdatabase();
				&savelanguagedatabase();
			}
			...
			while($sentence=pop(@sentences))
			{
				...
				if( ($commandline{'heuristic'} eq 'bigrams') || ($commandline{'heuristic'} eq 'trigrams') )
				{
				...
				if( $commandline{'heuristic'} eq 'trigrams' )
				{
					@trigrams = &sentenceToTrigrams($sentence);
					
					while($trigram=pop(@trigrams))
					{
						if(!defined($languageTrigramDatabase{$trigram}))
						{
							$languageTrigramDatabase{$trigram}=$languageUrlCount;
						}
						else
						{
							@mom=split(':', $languageTrigramDatabase{$trigram});
							push(@mom, $languageUrlCount) if(! grep(/$languageUrlCount/,@mom));
							$languageTrigramDatabase{$trigram}=join(':',@mom);
						}
						print '$languageTrigramDatabase{'.$trigram.'}='.$languageTrigramDatabase{$trigram}."\n" if $debug{'trigrams'};
					}
				}
			}
		}	
...
sub savelanguagedatabase
{
	print "Saving virtual to physical language databases:";
	`mv language.url.gdbm     language.url.gdbm.mv`;
	`mv language.word.gdbm    language.word.gdbm.mv`;
	`mv language.bigram.gdbm  language.bigram.gdbm.mv`;
	`mv language.trigram.gdbm language.trigram.gdbm.mv`;
	...
	%physicalLanguageUrlDatabase = ( %languageUrlDatabase );
	%physicalLanguageWordDatabase = ( %languageWordDatabase );
	%physicalLanguageBigramDatabase = ( %languageBigramDatabase );
	%physicalLanguageTrigramDatabase = ( %languageTrigramDatabase );
...
{
	local($sentence)=@_;
	local(@words,$word,$letter,@letters,$bigrams);
	undef @words;
	undef $word;
	undef @letters;
	undef $letter;
