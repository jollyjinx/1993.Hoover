...
OPTIONS :
    ...
    languages=[none|examine*] 
        ...
        contents=[none*|examine]
                If set, generates a file 'contents' which consists of entries:
                    URL:urlname
                    TIT:titles:...
                    FOL:href
                    FOL:href
...
while($_=pop(@ARGV))
{
	...
	$commandline{'contents'}=$1			if /^contents=(.*)/;
...
print	"Contents      :",$commandline{'contents'},"\n";
...
{	
	...
	if($commandline{'contents'} ne 'none')
	{
		open(CONTENTS,'>>contents'); select(CONTENTS);$|=1;
	}
	select(STDOUT);$|=1;
	while( $_ = <UNKNOWN> )
	{
...
{	
	local($contents)=@_;
	local($baseurl,@linksonthispage, @namesonthispage);
	undef @linksonthispage;
	undef @namesonthispage;

	$h = parse_html($contents);
	for (@{ $h->extract_links(qw('a')) })
	{
		($link, $element) = @$_;
		print STDERR "LINK:$link ELEMENT:$element\n";		
   }
	...
	# get all links
	s{<\s*A\s*[^>]*HREF="http:\/\/([^"]+)"[^>]*>([^<]*)</A\s*>}
	{ 
		if($1 && $2)
		{
			$fasel=$2;
			$fasel=~s/[\r\n]+//g;
			push(@linksonthispage,($1.'NAME'.$fasel));
		}  
	}giesx;
	# get the structure
