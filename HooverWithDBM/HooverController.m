...
- (void)readURLsFromStdinFilehandle;
{
    ...
    while( !feof(stdinstream) )
    {
        if( NULL != fgets(linebuffer, MAXIMUM_URL_LENGTH-1, stdinstream) )
        {
            ...
            while( 0 && [fetcherController count] > 30 )		// keep the Hoover not to busy just loading robot.txt files
            {
...
- (id)initWithConfiguration:(NSDictionary *)configurationDictionary;
{
    ...
    if( !(gdbmFile = [EODatabaseFile newEODatabaseFile:[generalConfiguration objectForKey:@"eomodel"] create:YES readOnly:NO]) )    
    {
        NSLog(@"HooverController: Couldn't create EODatabaseFile with database: %@", [generalConfiguration objectForKey:@"eomodel"]);
        return nil;
    }
